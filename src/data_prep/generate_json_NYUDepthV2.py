"""
Generates a JSON file organizing NYU Depth V2 HDF5 dataset files into train/val/test splits.

This script reads CSV files containing file paths for the NYU Depth V2 dataset,
randomly splits the training data into train/validation sets, and creates a 
structured JSON file that can be used by dataset loaders.

The output JSON format:
{
    "train": [{"filename": "train/dining_room_0031/02366.h5"}, ...], // 45,205 samples (95% of training data)
    "val": [{"filename": "train/bedroom_0026/00616.h5"}, ...],       // 2,379 samples (5% of training data)
    "test": [{"filename": "val/official/00001.h5"}, ...]             // 654 official test samples
}
"""
...
parser.add_argument('--csv_train', type=str, required=False,
                    default='src/data_prep/nyudepth_hdf5_train.csv',
                    help='Train data csv file')
parser.add_argument('--csv_test', type=str, required=False,
                    default='src/data_prep/nyudepth_hdf5_val.csv',
...
def main():
    ...
    assert (args.val_ratio >= 0.0) and (args.val_ratio <= 1.0), "Validation set ratio should be in [0, 1] but {}".format(args.val_ratio)
