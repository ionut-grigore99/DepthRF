...
# Check if download directory is provided
if [ -z "$1" ]; then
    echo "Error: Download directory not specified"
    echo "Usage: bash src/data_prep/raw_data_downloader.sh <DOWNLOAD_DIR>"
    exit 1
fi
...
for i in ${files[@]}; do
	...
	echo "Downloading: "$shortname
        sudo wget 'https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
        sudo unzip -o $shortname
        sudo rm $shortname
done