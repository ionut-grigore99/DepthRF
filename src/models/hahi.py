...
class HAHIHeteroNeck(BaseModule):
    ...
    def __init__(self,
        ...
        for in_channel, out_channel in zip(in_channels, out_channels):
            self.lateral_convs.append(ConvModule(in_channel, out_channel, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg))
    ...
    def forward(self, inputs):
        ...
        if self.self_att:
            src = self.self_attn(src_flatten, key=None, value=None, identity=None, query_pos=lvl_pos_embed_flatten, key_padding_mask=None, reference_points=reference_points, spatial_shapes=spatial_shapes, level_start_index=level_start_index,)
        else:
            src = src_flatten
        ...
        if self.cross_att:
            fusion_res_conv = self.multi_att(query, key=None, value=src, identity=None, query_pos=query_embed, key_padding_mask=None, reference_points=reference_points_input, spatial_shapes=spatial_shapes, level_start_index=level_start_index,)
        else:
        ...
        # unfold the features back to the origin shape
        start = 0
        fusion_res_trans = []
        for i in range(len(feats_trans)):
            bs, c, h, w = feats_trans[i].shape
