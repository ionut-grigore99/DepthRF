...
from src.utils import DEPTH_TRANSFORM, BaseDepthRefine

@HEADS.register_module()
class DDIMDepthEstimate_Swin_ADDHAHI(BaseDepthRefine):

    def __init__(self, up_scale_factor=1, inference_steps=20, num_train_timesteps=1000, return_indices=None,
            depth_transform_cfg=dict(type='DeepDepthTransformWithUpsampling', hidden=16, eps=1e-6), **kwargs):
        ...
        for i in range(len(in_channels)):
            self.conv_lateral.append(nn.Sequential(nn.Conv2d(in_channels[i], fpn_dim, 3, 1, 1, bias=False), build_norm_layer(dict(type='BN'), fpn_dim)[1], nn.ReLU(True)))

            if i != 0:
                self.conv_up.append(
                    nn.Sequential(
                        build_upsample_layer(upsample_cfg, in_channels=fpn_dim, out_channels=fpn_dim, kernel_size=2, stride=2),
    ...
    def forward(self, fp, depth_map, depth_mask, gt_depth_map=None, return_loss=False, **kwargs):
        ...
        refined_depth_t, = self.pipeline(batch_size=x.shape[0], device=x.device, dtype=x.dtype, shape=gt_map_t.shape[-3:],
            input_args=(x, None, None, None), num_inference_steps=self.diffusion_inference_steps, return_dict=False)
        refined_depth = self.depth_transform.inv_t(refined_depth_t)
        ddim_loss = self.ddim_loss(pred_depth=refined_depth, gt_depth=gt_map_t, refine_module_inputs=(x, None, None, None), blur_depth_t=refined_depth_t, weight=1.0)

        output = {'pred': refined_depth, 'pred_init': gt_map_t, 'blur_depth_t': gt_map_t , 'ddim_loss': ddim_loss, 'gt_map_t': gt_map_t, 
                'pred_uncertainty': None, 'pred_inter': None, 'weight_map': None, 'guidance': None, 'offset': None, 'aff': None, 'gamma': None, 'confidence': None}

        return output

    def loss(self, pred_depth, gt_depth, refine_module_inputs, blur_depth_t, pred_uncertainty=None, weight_map=None, **kwargs):
        ...
        for loss_cfg in self.loss_cfgs:
            ...
            if loss_key == 'ddim_loss':
                ...
                continue
            loss = loss_fnc(pred_depth=pred_depth, pred_uncertainty=pred_uncertainty, gt_depth=gt_depth, refine_module_inputs=refine_module_inputs, blur_depth_t=blur_depth_t, weight_map=weight_map, **loss_cfg, **kwargs)
...
class CNNDDIMPipiline:
    ...
    def __call__(self, batch_size, device, dtype, shape, input_args, generator: Optional[torch.Generator] = None, eta: float = 0.0,
            num_inference_steps: int = 50, return_dict: bool = True, **kwargs) -> Union[Dict, Tuple]:
        ...
        for t in self.scheduler.timesteps:
            ...
            # 2. predict previous mean of image x_t-1 and add variance depending on eta; eta corresponds to η in paper and should be between [0, 1]
            # do x_t -> x_t-1
