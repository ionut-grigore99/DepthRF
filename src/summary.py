...
class DepthRF_Summary(BaseSummary):
    def __init__(self, log_dir, mode, conf, loss_name, metric_name):
        assert mode in ['train', 'val', 'test'], "mode should be one of ['train', 'val', 'test'] but got {}".format(mode)

        super(DepthRF_Summary, self).__init__(log_dir, mode, conf)
    ...
    def update(self, global_step, sample, output):
        ...
        dep = sample['depth_sparse'].detach().data.cpu().numpy()
        gt = sample['depth_gt'].detach().data.cpu().numpy()
        ...
        if num_summary > self.conf['num_summary']:
            num_summary = self.conf['num_summary']
        ...
        dep = np.clip(dep, a_min=0, a_max=self.conf['max_depth'])
        gt = np.clip(gt, a_min=0, a_max=self.conf['max_depth'])
        pred = np.clip(pred, a_min=0, a_max=self.conf['max_depth'])
        if 'refineddepth' in output.keys():
            refineddepth = np.clip(refineddepth, a_min=0, a_max=self.conf['max_depth'])
        ...
        for b in range(0, num_summary):
            ...
            dep_tmp = 255.0 * dep_tmp / self.conf['max_depth']
            gt_tmp = 255.0 * gt_tmp / self.conf['max_depth']
            pred_tmp = 255.0 * pred_tmp / self.conf['max_depth']
            if 'refineddepth' in output.keys():
                refineddepth_tmp = 255.0 * refineddepth_tmp / self.conf['max_depth']
            ...
            img = np.concatenate((rgb_tmp, dep_tmp, pred_tmp, gt_tmp, confidence_tmp), axis=1)
    ...
    def save(self, epoch, idx, sample, output):
        with torch.no_grad():
            if self.conf['save_result_only']:
                ...
                if self.conf['save_raw_npdepth']:
                    np.save(raw_save_pred, raw_depth)
            else:
                # Parse data

                rgb = sample['rgb'].detach()
                dep = sample['depth_sparse'].detach()
                pred = output['pred'].detach()
                gt = sample['depth_gt'].detach()
                depth_map = sample['depth_filled'].detach()
                ...
                dep = dep / self.conf['max_depth']
                pred = pred / self.conf['max_depth']
                if 'refineddepth' in output.keys():
                    refineddepth = refineddepth / self.conf['max_depth']
                pred_gray = pred
                gt = gt / self.conf['max_depth']
                depth_map = depth_map / self.conf['max_depth']
                ...
                # feat_init = feat_init / self.conf['max_depth']
                ...
                if list_feat is not None:
                    for k in range(0, len(list_feat)):
                        ...
                        feat_inter = feat_inter / self.conf['max_depth']
                ...
                if list_feat is not None:
                    for k in range(0, len(list_feat)):
                        path_save_inter = '{}/04_pred_prop_{:02d}.png'.format(self.path_output, k)
